...
class BCOrcaAgent:
    def __init__(self, id: int, pos: Vector2D, goal: Vector2D, **kwargs):
        ...
        self.M: Vector2D = kwargs.get('M', Vector2D(default_m, default_m))
    ...
    def optimize_alpha_from_u(self, u, p_est, m_est, alpha_prior, lambda_max=2.0, lambda_min=0.1, k=1.0):
        ...
        基于避障向量u优化责任系数alpha
        参数：
            u: 避障向量（Vector2D，numpy数组，shape=(3,)）
            M_self: 自身质量（已知）
    ...
    def _estimate_neighbor_properties(self, other: 'BCOrcaAgent') -> Tuple[float, Vector2D, float]:
        """估计邻居 'other' 的 P, M"""
        ...
        for pos, vel, _ in other.history:
            ...
            if not goal_dir.is_zero() and not vel_dir.is_zero():
                alignments.append(max(0, vel_dir@goal_dir)) # clip at 0
        ...
        if avg_accel > cfg.EPSILON:
            ...
            m_est = Vector2D(m_est_mag, m_est_mag)
        else:
            m_est = other.M # 无法估计，返回默认值

        return p_est, m_est
    
    def _predict_neighbor_intention(self, other: 'BCOrcaAgent') -> Tuple[Vector2D, float]:
    ...
    def solve_fast_brain(self, other:'BCOrcaAgent'):
        ...
        v_radial = v_rel@d_hat # 计算径向速度 (判断靠近/远离)
        
        if v_rel_norm > cfg.EPSILON:
            cross_product = Vector2D.det(d_vec, v_rel)
            d_cpa = abs(cross_product) / v_rel_norm
        else:
    ...
    def compute_new_velocity(self, time_step):
        ...
        for other in self.agent_neighbors:
            ...
            line.point = self.vel + alpha_hybrid * u
