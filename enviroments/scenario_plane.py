...
class PlaneScenario:
    ...
    def _get_center(self) -> Vector2D:
        """根据平面设置场景中心（固定一个坐标）"""
        return Vector2D(cfg.WORLD_SIZE[0]/2, cfg.WORLD_SIZE[1]/2)

    def _generate_opposing_positions(self) -> Tuple[List[Vector2D], List[Vector2D]]:
        """
        通用对向点生成：
        1. 均匀分配N/2个角度（0~2π）
        2. 每个角度生成一对对称点（中心两侧）
    ...
    def create_agents_with_heterogeneity(self, hetero_type: str) -> List[Agent]:
        ...
        if hetero_type == "M_diff":
            ...
            Ms = [np.eye(2) for _ in range(self.num_agents//2)] + \
                 [np.eye(2)*10 for _ in range(self.num_agents - self.num_agents//2)]
            Ps = [50 for _ in range(self.num_agents)]  # P相同
        
        elif hetero_type == "P_diff":
            # P不同：前半低优先级（1），后半高优先级（100），M相同
            Ms = [np.eye(2) for _ in range(self.num_agents)]  # M相同
