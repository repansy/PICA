...
class Agent:
    ...
    def __init__(self, id: int, pos: Vector3D, goal: Vector3D, inertia_matrix: np.ndarray, priority: float = 1.0):
        ...
        # --- 算法内部状态 ---
        self.pref_velocity = Vector3D(0, 0, 0)
        self.agent_neighbors: List['Agent'] =  []
    ...
    def compute_new_velocity(self, dt: float) -> Vector3D:
        """
        计算新的速度，核心决策函数
        参数:
        ...
        # 1. 初始化和邻居筛选
        v_pref = self._break_deadlock(self.pref_velocity, self.agent_neighbors)
        self._update_local_density(self.agent_neighbors)

        # 2. 预处理
        initial_risk_scores = {n.id: self._calculate_risk(self, n) for n in self.agent_neighbors}
        sorted_neighbors = sorted(self.agent_neighbors, key=lambda n: initial_risk_scores[n.id], reverse=True)
        ...
        for neighbor in critical_neighbors:
            ...
            self._optimize_alpha_hybrid(neighbor, risk_score, effective_states, v_pref)
        ...
        # 4. 求解最优速度 (使用标准的RVO2求解器)
        fail_plane, new_velocity = linear_program3(constraints, self.max_speed, v_pref, False)

        if fail_plane < len(constraints):
            new_velocity = linear_program4(constraints, fail_plane, self.max_speed, new_velocity)
    ...
    def _optimize_alpha_hybrid(self, neighbor: 'Agent', risk_score: float, effective_states: Dict, v_pref_i: Vector3D):
        """
        ...
        alpha_analytical = self._calculate_analytical_alpha(neighbor, effective_states, v_pref_i)
    ...
    def _calculate_analytical_alpha(self, neighbor: 'Agent', states: Dict, v_pref_i: Vector3D) -> float:
        """
        ...
        返回:
            float: 解析法责任值α
        """
        v_pref_j = neighbor.pref_velocity
        ...
        # 1.3 求解两种责任下的速度
        _, v_i_at_0 = linear_program3([constraint_i_at_0], self.max_speed, v_pref_i, False)
        _, v_i_at_delta = linear_program3([constraint_i_at_delta], self.max_speed, v_pref_i, False)
        ...
        _, v_j_at_0 = linear_program3([constraint_j_at_0], self.max_speed, v_pref_j, False)
        _, v_j_at_delta = linear_program3([constraint_j_at_delta], self.max_speed, v_pref_j, False)
    ...
    def compute_neighbors(self, all_agents: List['Agent']) -> List['Agent']:
        """
        过滤出在交互范围内的邻居
        参数:
            all_agents: 所有智能体列表   
        ...
        self.agent_neighbors = neighbors
    ...
    def _create_orca_halfspace_from_states(self, alpha: float, states: Dict) -> Optional[Dict]:
        ...
        inv_time_horizon = 1.0 / cfg.TTC_HORIZON

        u = Vector3D()
        normal = Vector3D()

        if dist_sq > combined_radius_sq: # 非碰撞
            ...
            if dot_product < 0.0 and dot_product**2 > combined_radius_sq * w_length_sq:
                ...
                c = rel_vel.norm_sq() - (rel_pos.norm_sq() * rel_vel.norm_sq() - b**2) / (dist_sq - combined_radius_sq)
                
                discriminant = b**2 - a * c
                if discriminant < 0: return None 
                
                t = (b + math.sqrt(discriminant)) / a
                ww = rel_vel - t * rel_pos
                ww_length = ww.norm()
                unit_ww = ww / ww_length
                normal = unit_ww
                u = (combined_radius * t - ww_length) * unit_ww
        else: # 碰撞
            inv_time_step = 1.0 / cfg.TIMESTEP
        ...
        my_responsibility = alpha
        other_responsibility = 1.0 - my_responsibility
        
        plane_point = vel_i + other_responsibility * u
        
        return Plane(normal=normal, point=plane_point)
